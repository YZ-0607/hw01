...
def train(model, train_loader, criterion, optimizer, device, epochs=5):
    ...
    for epoch in range(epochs):  # 修正：添加空格，改为 range
        running_loss = 0.0
        correct = 0
        total = 0
      
        for i, (images, labels) in enumerate(train_loader):  # 修正：添加空格，改为 enumerate
            # 将数据移动到指定设备
...
def display_data(data_loader, num_images=25):
    ...
    # 显示图像
    for i in range(min(num_images, images.shape[0])):  # 修正：添加空格，改为 range
